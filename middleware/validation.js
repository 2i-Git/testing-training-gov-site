...
const validators = {
  ...
  businessType: (value) => {
    const validTypes = ['pub', 'restaurant', 'bar', 'nightclub', 'hotel', 'off-licence', 'supermarket', 'other'];
...
const personalDetailsValidation = [
  ...
  body('dobDay')
    .isInt({ min: 1, max: 31 }).withMessage('Enter a valid day'),

  body('dobMonth')
    .isInt({ min: 1, max: 12 }).withMessage('Enter a valid month'),

  body('dobYear')
    .isInt({ min: 1900, max: new Date().getFullYear() - 18 }).withMessage('Enter a valid year'),
  ...
  body('addressLine1')
    ...
    .isLength({ min: 1, max: 100 }).withMessage('Address line 1 must be between 1 and 100 characters'),
  ...
  body('addressTown')
    ...
    .isLength({ min: 1, max: 50 }).withMessage('Town or city must be between 1 and 50 characters'),

  body('addressCounty')
    .optional({ checkFalsy: true })
    .trim()
    .isLength({ max: 50 }).withMessage('County must not exceed 50 characters'),
...
const businessDetailsValidation = [
  ...
  body('businessAddressLine1')
    .trim()
    .notEmpty().withMessage('Business address line 1 is required')
    .isLength({ min: 1, max: 100 }).withMessage('Business address line 1 must be between 1 and 100 characters'),

  body('businessAddressLine2')
    .optional({ checkFalsy: true })
    .trim()
    .isLength({ max: 100 }).withMessage('Business address line 2 must not exceed 100 characters'),

  body('businessAddressTown')
    .trim()
    .notEmpty().withMessage('Business town or city is required')
    .isLength({ min: 1, max: 50 }).withMessage('Business town or city must be between 1 and 50 characters'),

  body('businessAddressCounty')
    .optional({ checkFalsy: true })
    .trim()
    .isLength({ max: 50 }).withMessage('Business county must not exceed 50 characters'),

  body('businessAddressPostcode')
    .trim()
    .notEmpty().withMessage('Business postcode is required')
    .custom(value => {
      if (!validators.ukPostcode(value)) {
  ...
  body('businessEmail')
    .optional({ checkFalsy: true })
    .trim()
    .isEmail().withMessage('Enter a valid business email address')
    .normalizeEmail()
];
...
const licenseDetailsValidation = [
  ...
  body('premisesType')
    .notEmpty().withMessage('Premises type is required')
    .custom(value => {
  ...
  body('premisesAddressLine1')
    .trim()
    .notEmpty().withMessage('Premises address line 1 is required')
    .isLength({ min: 1, max: 100 }).withMessage('Premises address line 1 must be between 1 and 100 characters'),

  body('premisesAddressLine2')
    .optional({ checkFalsy: true })
    .trim()
    .isLength({ max: 100 }).withMessage('Premises address line 2 must not exceed 100 characters'),

  body('premisesAddressTown')
    .trim()
    .notEmpty().withMessage('Premises town or city is required')
    .isLength({ min: 1, max: 50 }).withMessage('Premises town or city must be between 1 and 50 characters'),

  body('premisesAddressCounty')
    .optional({ checkFalsy: true })
    .trim()
    .isLength({ max: 50 }).withMessage('Premises county must not exceed 50 characters'),

  body('premisesAddressPostcode')
    .trim()
    .notEmpty().withMessage('Premises postcode is required')
    .custom(value => {
      if (!validators.ukPostcode(value)) {
  ...
  body('activities')
    .custom((value, { req }) => {
      if (!req.body.activities || (Array.isArray(req.body.activities) && req.body.activities.length === 0)) {
        throw new Error('Select at least one activity');
      }
      
      const validActivities = [
        'sale-on', 'sale-off', 'regulated-entertainment', 
        'late-night-refreshment', 'live-music', 'recorded-music'
      ];
      
      const activities = Array.isArray(req.body.activities) ? req.body.activities : [req.body.activities];
...
const declarationValidation = [
  body('declaration')
    .notEmpty().withMessage('You must confirm that the information you have provided is correct')
    .equals('yes').withMessage('You must confirm the declaration to continue')
];
