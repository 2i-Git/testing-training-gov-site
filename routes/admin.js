...
router.post('/login', (req, res) => {
  ...
  res.render('admin-login', { error: 'Invalid email or password', csrfToken: res.locals.csrfToken });
});
...
router.post('/applications/:id/status', requireAdmin, asyncHandler(async (req, res) => {
  ...
  try {
    ...
    // Add success message
    const successMessage = status === 'approved' ? 'Application approved successfully' : 'Application rejected successfully';
    res.redirect('/admin/applications?success=' + encodeURIComponent(successMessage));
  } catch (error) {
    console.error('Error updating application status:', error);
    res.redirect('/admin/applications?error=' + encodeURIComponent('Failed to update application status'));
  }
}));
